...
if (isset($_GET['del'])) { //删除班级信息
  ...
  exit(0);
} else if (isset($_POST['year'])) { //增加班级写入数据库
  ...
  for ($i = 0; $i < $class_num; $i++) {
    if (class_is_exist($class_list[$i])) {
      echo $class_list[$i] . "-有重名，无法写入！<br>\n";
    } else {
  ...
  exit(0);
} else if (isset($_POST['cid'])) { //班级资料写入数据库，保存后资料后跳转回班级列表
  require_once("../include/check_post_key.php");
...
<div class="am-avg-md-1" style="margin-top: 20px; margin-bottom: 20px;width:800px;">
  <form class="am-form am-form-horizontal" action="class_edit.php" method="post">
    ...
    <div class="am-form-group" style="white-space: nowrap;">
      ...
      <div class="am-u-sm-8">
        <input type="text" style="width:340px;" maxlength="20" id="new_name" name="new_name" placeholder="限20个以内的汉字、字母、数字或下划线" pattern="^[\u4e00-\u9fa5_a-zA-Z0-9]{1,20}$" required />
      </div>
    </div>
