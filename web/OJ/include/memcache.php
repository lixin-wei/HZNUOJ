<?php
    ...
    //否则简单地返回false
    # Gets key / value pair into memcache … called by $mysqli->query_cache()
    function getCache($key) {
        global $memcache;
        return ($memcache) ? $memcache->get($key) : false;
    }

    # Puts key / value pair into memcache … called by $mysqli->query_cache()
    function setCache($key, $object, $timeout = 60) {
        global $memcache;
        return ($memcache) ? $memcache->set($key,$object,MEMCACHE_COMPRESSED,$timeout) : false;
    }

    # Caching version of $mysqli->query()
    function $mysqli->query_cache($sql, $linkIdentifier = false,$timeout = 4) {
...
//memcached处理时输出的相关信息
        if (!($cache = getCache(md5("$mysqli->query" . $sql)))) {

            $cache = false;

            $r = ($linkIdentifier != false) ? $mysqli->query($sql,$linkIdentifier) : $mysqli->query($sql);

   //读取数据库，并将结果放入$cache数组中
            if (is_resource($r) && (($rows = $r->num_rows) != 0)) {
                for ($i=0;$i<$rows;$i++) {
    ...
    //如果开启了服务器的话，数据将会被缓存到memcached服务器中
                if (!setCache(md5("$mysqli->query" . $sql), $cache, $timeout)) {
